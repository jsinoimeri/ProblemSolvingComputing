...
/*
I think there is a inconsistency in the sample provided:
    if a gap is 60 mins or greater it is allowed if previous and actual times
    are within a day; however it is not allowed if midnight is in the gap.
    Ex: 23:00 - 01:00. This 2 hour gap is not allowed; however
        14:00 - 16:00. This 2 hour gap is allowed
    
    I made my assignment to match the above behaviour.
    However to fix it I introduced the alternative code 
    marked by //*-*    
*/
...
//*-*   const int MAX_GAP_ALLOWED = 5*60;
...
//*-*           if (valid_times > 0 && actual_mins - previous_mins > MAX_GAP_ALLOWED) {
